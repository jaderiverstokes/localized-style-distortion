# Generates one-style background-masked images
...
out_file = lambda s: 'images/output-images/'+input_name+'/'+input_name+'-bg-masked-'+ s +'.png'
...
#m = cv2.imread(bin_mask_file).astype(float) / 255.0
...
for s in styles:
	...
	fg = cv2.multiply(1.0-m, style)
	bg = cv2.multiply(m, orig)
	im_out = fg + bg
